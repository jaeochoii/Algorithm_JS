...
class MinHeap extends Heap {
    ...
    // 최소 힙 : 오름차순 정렬 / 최대 힙 : 내림차순 정렬 / 불안정 정렬(같은 값 순서유지 안됨)
    sort() {
        let sort = [];
...
function solution(n, works) {
    ...
    const condition = works.reduce((sum, currValue) => sum + currValue, 0)
    if (condition <= n) return 0;
    ...
    while (n > 0) {
        worksHeap.add(worksHeap.poll() - 1)
        n -= 1;
    }
    return worksHeap.items.reduce((sum, currValue) => sum + currValue ** 2, 0);
}